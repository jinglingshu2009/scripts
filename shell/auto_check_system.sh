  ...
  do  
   case $_ip_list in 
		...
		HM65-A)	
			_network=`./auto_ssh.sh ssh $_ip_list $_passwd 'ifconfig -a |grep Ethernet |wc -l'`
			 if [ -z $_network ];then 
			   _name3=`$_ip_list=start error!`
			   break
			  if [ "$_network" == "12" ];then
			   _name3=`$_ip_list=$_network`
			   #./auto_ssh.sh ssh $_ip_list $_passwd 'reboot'
			else 
			   _name3=`$_ip_list=$_network error!`
			   fi
			;;		
		HM65-B)
			_network=`./auto_ssh.sh ssh $_ip_list $_passwd 'ifconfig -a |grep Ethernet |wc -l'`
			 if [ -z $_network ];then 
			   _name4=`$_ip_list=start error!`
			   break
			if [ "$_network" == "12" ];then
			   _name4=`$_ip_list=$_network`
			   #./auto_ssh.sh ssh $_ip_list $_passwd 'reboot'
			else 
			   _name4=`$_ip_list=$_network error!`
			   fi
			;;
	esac
done
