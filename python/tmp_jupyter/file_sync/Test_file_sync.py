...
def touch_file():
    for num in range(1, file_num + 1):
        ...
        for file_type_num in range(len(_list)):
            ...
            # 获取文件单位（size/KB/MB/GB），根据单位生成等量级的大小文件
            ...
            if tmp_unit == "Z":
                ...
                now_filename = "%s%s%s%s.%s" % (file_path,os.sep,tmp_name.split(".")[0], file_md5, tmp_name.split(".")[1])
                ...
                # 获取文件大小数值
                tmp_size = file_size.upper().split(tmp_unit)[0]
                # 按大小生成指定量级((MB)大小文件(小于10MB使用字符填充，大于使用内存随机值填充)
                if tmp_size < "10":
                    M_size = tmp_name + "CBXA" * 256 * 1024 * int(tmp_size)
                    with open(tmp_name, "w") as f:
                        f.write(M_size)
                        f.close()
                else:
                ...
                now_filename = "%s%s%s.%s" % (file_path,os.sep,tmp_name.split(".")[0], tmp_name.split(".")[1])
                # 生成指定量级((MB)大小文件
                fp = open(now_filename, "w")
                fp.write(tmp_name)
                fp.truncate(G_size)
                fp.close()
            add_filename=new_filename


############################################################
file_type="exe"
file_path="C:\Users\Administrator\Desktop\\test"
file_size="20MB"   #SIZE|KB|MB|GB
file_num=100
check_md5 = "on"   #on|off
    ...
    for del_file in range (len(file_list)/2):
        if check_md5 == "on":
            ...
            if file_md5 == old_file_md5:
                ...
                print "%s  %s  %s  %s  %s" % (del_file + 1, time,"delete", "md5_check=ok", file_path + os.sep + file_list[0])
            else:
                print "%s  %s  %s  %s  %s" % (del_file + 1,time,"delete", "md5_check=error", file_path + os.sep + file_list[0])
                # print "校验文件MD5失败，请排查文件%s" %  file_list[0]
                pass
        else:
            # 关闭文件MD5校验
            os.remove(file_path + os.sep + file_list[0])
            print "%s   %s  %s  %s" % (del_file + 1, "delete", "no_check=ok", file_path + os.sep + file_list[0])
        del file_list[0]
elif sync_mode.upper() == "RECV":
    # 同步模式：接收（RECV）
    for del_file in range (len(file_list)/2):
        if check_md5 == "on":
            # 开启文件MD5校验
            ...
            if file_md5 == old_file_md5:
                os.remove(file_path + os.sep + file_list[0])
                print "%s  %s  %s  %s  %s" % (
                del_file + 1, time, "delete", "md5_check=ok", file_path + os.sep + file_list[0])
            else:
                print "%s  %s  %s  %s  %s" % (
                del_file + 1, time, "delete", "md5_check=error", file_path + os.sep + file_list[0])
                # print "校验文件MD5失败，请排查文件%s" %  file_list[0]
                pass
        else:
            # 关闭文件MD5校验
